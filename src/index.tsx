...
createServer({
    ...
    routes() {
        ...
        this.get('/genres', () => this.schema.all('genre'))
        // this.get('/movies', () => this.schema.all('movie'))
        this.get('/movies', (schema, request) =>{
            let { Genre_id } = request.queryParams
            return schema.movies.where({ 'Genre_id': Genre_id})
        })
        this.get('/genres/:id', (schema, request) => {
                let id = request.params.id
                return schema.find('genre', id)
            })
    }
})
